...
class EnterpriseInstance(DeploymentInstance):
    def setup(self, already_deployed=False):
        ...
        # # ActiveDir:    192.168.200.3      <-- Samba RCE vulnerability (or - currently - netcat reverse shell)
        # # CEO:          192.168.200.4 FLAG <-- SSH password login, sudoers writeable
        # # Finance:      192.168.200.5 FLAG <-- VSFTP Backdoor vulnerability
        # $ HR:           192.168.200.6
        # ! Intern:       192.168.200.7      <-- infected with attacker (SSH password login)
        # # Database:     192.168.201.3 FLAG <-- SSH password login, weak user password (or netcat reverse shell)

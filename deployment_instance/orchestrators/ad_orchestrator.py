...
class ADOrchestrator(Orchestrator):
    ...
    def samba_AD(self, host: str, samba_workgroup: str, samba_realm: str, realm_password: str):
        task = OrchestrationTask(host, "samba-AD")
        task.set_params({'samba_workgroup': samba_workgroup, 'samba_realm': samba_realm, 'realm_password': realm_password})
        self.add_task(task)

    def add_Groups(self, host: str):
        task = OrchestrationTask(host, "addGroups")
        self.add_task(task)
    ...
    def add_Users(self, host: str, user_details_file: str):
        ...
        task = OrchestrationTask(host, "addUsers")
        task.set_params({'user_details': user_details})
        self.add_task(task)

    def join_AD(self, host: str):
        task = OrchestrationTask(host, "join-AD")
        self.add_task(task)
